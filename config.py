...
class Config:
    def __init__(self):
        ...
        # ========== 🍪 COOKIES FIX - START ==========
        # 📌 1. cookies.txt ဖိုင်ကိုဖတ်ပါ
        self.COOKIE_FILE = "cookies.txt"
        self.COOKIES = {}
        
        # cookies.txt ဖိုင်ရှိရင် ဖတ်ပါ
        if os.path.exists(self.COOKIE_FILE):
            try:
                with open(self.COOKIE_FILE, 'r') as f:
                    content = f.read()
                    # Netscape format ကို parse လုပ်ပါ
                ...
                print(f"✅ Loaded {len(self.COOKIES)} cookies from {self.COOKIE_FILE}")
            except Exception as e:
                print(f"⚠️ Failed to parse cookies.txt: {e}")
        else:
            print("⚠️ cookies.txt not found, using default cookies")

        # 📌 2. COOKIES_URL ကိုလည်းဖတ်ပါ
        ...
        # 📌 3. ဘာ cookie မှမရရင် default ထည့်ပါ
        if not self.COOKIES and not self.COOKIES_URL:
            self.COOKIES = {
